...
export class PoloReport60mComponent implements OnInit {
  private cacheExist: boolean = this._cacheService.exists('cache_60m');
  public coins: Array<any> = this.cacheExist ? this._cacheService.get('cache_60m') : [];
  ...
  public callApiGetData() {
    ...
    this.poloService.getDataFromTable('kevin_returnTicker_1h').then(data => {
      ...
      this._cacheService.set('cache_60m', data, { maxAge: 300 * 60 });
    });
  }
  public lastData() {
    this.callApiGetData();
